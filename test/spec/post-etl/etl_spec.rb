RSpec.describe "etl" do
  describe "postgres users table" do
    it "successfully filters out rows with NULL salesRepEmployeeNumber" do
      # thing = mysql.query("SELECT * FROM classicmodels.customers WHERE customerName = 'Havel & Zbyszek Co'")
      # byebug
      ...
      result = pg.exec("SELECT * FROM users WHERE customerName = 'Havel & Zbyszek Co'").first
